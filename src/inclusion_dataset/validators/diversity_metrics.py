...
from ..config.constants import METRICS, LIMITS, PATTERNS, VALIDATION
from .exceptions import (
    DiversityValidationError, LexicalAnalysisError, 
...
class DiversityMetrics:
        ...
        for data_path, download_name in required_data:
            try:
                ...
                try:
                    ...
                    raise ConfigurationError(f"Failed to download NLTK data '{download_name}': {e}")
    ...
    def calculate_lexical_diversity(self, texts: List[str]) -> Dict[str, float]:
        ...
        """
        if not texts:
            raise InsufficientDataError("No texts provided for lexical diversity analysis")
    ...
    def detect_template_patterns(self, instructions: List[str]) -> Dict[str, Any]:
        ...
        """
        if not instructions:
            raise InsufficientDataError("No instructions provided for template detection")
    ...
    def _calculate_ngram_overlaps(self, instructions: List[str]) -> Dict[str, float]:
        ...
        """
        return {
            "unigram_overlap": self._calculate_ngram_overlap(instructions, PATTERNS.UNIGRAM_SIZE),
            "bigram_overlap": self._calculate_ngram_overlap(instructions, PATTERNS.BIGRAM_SIZE),
            "trigram_overlap": self._calculate_ngram_overlap(instructions, PATTERNS.TRIGRAM_SIZE)
        }
    
    def measure_semantic_spread(self, instructions: List[str], max_clusters: int = None) -> Dict[str, Any]:
        ...
        """
        return self.semantic_analyzer.measure_semantic_spread(instructions, max_clusters)
    ...
    def _calculate_mtld(self, tokens: List[str], ttr_threshold: float = None) -> float:
        ...
        try:
            ...
            backward_mtld = self._calculate_directional_mtld(tokens[::-1], ttr_threshold)
    ...
    def _calculate_directional_mtld(self, tokens: List[str], threshold: float) -> float:
        ...
        if current_segment:
            ...
            segments += remaining_ttr / threshold if threshold > 0 else 0
    ...
    def _calculate_maas_index(self, tokens: List[str], types: Set[str]) -> float:
        ...
        try:
            maas = (math.log(len(tokens)) - math.log(len(types))) / (math.log(len(tokens)) ** 2)
            return maas
    ...
    def _calculate_ngram_overlap(self, texts: List[str], n: int) -> float:
        ...
        try:
            ...
            raise LexicalAnalysisError(f"N-gram overlap calculation failed for n={n}: {e}")
    ...
    def _find_common_patterns(self, instructions: List[str], min_length: int = 3) -> List[str]:
        ...
        filtered_patterns = [
            p for p in patterns 
            if len(p.split()) > 1 and not all(word in self.stopwords for word in p.split())
        ]
    ...
    def calculate_syntactic_diversity(self, instructions: List[str]) -> Dict[str, Any]:
        ...
        """
        if not instructions:
            return {"sentence_patterns": 0, "avg_sentence_length": 0.0, "complexity_distribution": {}}
    ...
    def _extract_sentence_pattern(self, sentence: str) -> str:
        ...
        if any(word in lower_sentence for word in ['können', 'sollen', 'müssen', 'dürfen', 'mögen']):
            pattern += "_MODAL"
