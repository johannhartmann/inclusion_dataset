...
from .exceptions import InsufficientDataError, TemplateValidationError
...
class TemplateDetector:
    ...
    def __init__(self, max_template_overlap: Optional[float] = None):
        ...
        self.detected_patterns: List[TemplatePattern] = []
    ...
    def _find_structural_patterns(
        ...
        for structure, count in structure_counts.items():
            if count > max(2, len(instructions) * 0.1):  # More than 10% or at least 3
                examples = [
                    inst for struct, inst in structural_forms if struct == structure
                ][:3]
                patterns.append(
                    TemplatePattern(
    ...
    def _find_prefix_suffix_patterns(
        self, instructions: List[str]
    ) -> List[TemplatePattern]:
    ...
    def _find_placeholder_patterns(
        self, instructions: List[str]
    ) -> List[TemplatePattern]:
        ...
        threshold = max(
            VALIDATION.MIN_INSTRUCTIONS_FOR_ANALYSIS,
        ...
        for pattern, count in pattern_counts.items():
            if count > threshold and pattern != instruction:
                examples = [
                    inst for pat, inst in placeholder_patterns if pat == pattern
                ][: PATTERNS.MAX_EXAMPLES_PER_PATTERN]
                patterns.append(
                    TemplatePattern(
    ...
    def _extract_structure(self, instruction: str) -> str:
        ...
        structure = re.sub(
            r"\b[a-z]+ung\b", "[NOUN_UNG]", structure
        )  # German -ung nouns
        structure = re.sub(
            r"\b[a-z]+heit\b", "[NOUN_HEIT]", structure
        )  # German -heit nouns
        structure = re.sub(
            r"\b[a-z]+keit\b", "[NOUN_KEIT]", structure
        )  # German -keit nouns
        structure = re.sub(
            r"\b[a-z]+isch\b", "[ADJ_ISCH]", structure
        )  # German -isch adjectives
        structure = re.sub(
            r"\b[a-z]+lich\b", "[ADJ_LICH]", structure
        )  # German -lich adjectives
    ...
    def _create_placeholder_pattern(self, instruction: str) -> str:
        ...
        pattern = re.sub(
            r"\b(?:Gender|Diversity|Inklusion|Behinderung|Migration)\b",
            "[DOMAIN_TERM]",
            pattern,
        )
    ...
    def _calculate_violation_score(
        self, instructions: List[str], patterns: List[TemplatePattern]
    ) -> float:
    ...
    def _generate_recommendations(
        self, violation_score: float, patterns: List[TemplatePattern]
    ) -> List[str]:
        ...
        recommendations.append(
            f"⚠ Template violation detected: {violation_score:.1%} > {self.max_template_overlap:.1%}"
        )
        ...
        if pattern_types.get("STRUCTURAL", 0) > 0:
            recommendations.append(
                "• Vary sentence structures (questions, statements, imperatives)"
            )

        if pattern_types.get("PREFIX", 0) > 0:
            recommendations.append(
                "• Use diverse instruction openings instead of repeated prefixes"
            )
        ...
        if pattern_types.get("SEMANTIC", 0) > 0:
            recommendations.append(
                "• Increase semantic diversity in instruction meanings"
            )
    ...
    def should_reject_batch(self, instructions: List[str]) -> Tuple[bool, str]:
        ...
        if results["templates_detected"]:
            ...
            return (
                True,
                f"Template violation: {violation_score:.1%} exceeds limit of {self.max_template_overlap:.1%}",
            )
