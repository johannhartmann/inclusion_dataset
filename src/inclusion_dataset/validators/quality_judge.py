...
- Fachliche Richtigkeit"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte für Qualitätsbewertung von inklusiven Sprachdaten.",
                    },
            ...
            assessment_data = self._extract_json_from_response(response_text or "")
    ...
    def filter_by_quality(
        self, samples: List[Dict], assessments: List[QualityAssessment]
    ...
    def generate_quality_report(
        ...
        return {
            ...
            "average_scores": {
                ...
                "instruction_text_fit": sum(instruction_fit_scores)
                / len(instruction_fit_scores),
                "response_quality": sum(response_quality_scores)
                / len(response_quality_scores),
    ...
    def _create_fallback_assessment(
        ...
        if not any(
            ...
            for word in ["inklusiv", "gender", "neutral", "barrierefrei"]
        ):
            score -= 1.0
    ...
    def _generate_improvement_recommendations(
        ...
        avg_scores = {
            "instruction_fit": sum(a.instruction_text_fit for a in assessments)
            / len(assessments),
            "response_quality": sum(a.response_quality for a in assessments)
            / len(assessments),
        ...
        if avg_scores["instruction_fit"] < 7.0:
            recommendations.append(
                "Improve instruction-text alignment by better contextual analysis"
            )

        if avg_scores["response_quality"] < 7.0:
            recommendations.append(
                "Enhance response quality with more specific and actionable suggestions"
            )

        if avg_scores["consistency"] < 7.0:
            recommendations.append(
                "Ensure consistency between problem identification and solutions"
            )

        if avg_scores["completeness"] < 7.0:
            recommendations.append(
                "Address all relevant inclusion aspects systematically"
            )
        ...
        if pass_rate < 0.8:
            recommendations.append(
                "Overall pass rate is low - review generation parameters"
            )
