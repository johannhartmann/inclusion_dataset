...
class Config(BaseModel):
    ...
    # Dataset parameters
    total_samples: int = Field(default=20000, description="Total number of samples to generate")
    transformation_ratio: float = Field(default=0.5, description="Ratio of transformation vs evaluation tasks")
    min_text_length: int = Field(default=150, description="Minimum text length in words")
    max_text_length: int = Field(default=300, description="Maximum text length in words")
    
    # Model configuration
    teacher_model: str = Field(default="gpt-4o", description="Teacher model for generation")
    openai_api_key: Optional[str] = Field(default=None, description="OpenAI API key")
    
    # Quality control
    min_quality_score: float = Field(default=7.0, description="Minimum quality score for acceptance")
    ...
    lexical_diversity_threshold: float = Field(default=0.4, description="Minimum TTR threshold")
    min_unique_unigrams: float = Field(default=0.8, description="Minimum unique unigram ratio")
    min_unique_bigrams: float = Field(default=0.9, description="Minimum unique bigram ratio")
    min_unique_trigrams: float = Field(default=0.95, description="Minimum unique trigram ratio")
    max_template_overlap: float = Field(default=0.05, description="Maximum allowed template overlap")
    min_semantic_distance: float = Field(default=0.3, description="Minimum cosine distance between instructions")
    ...
    formality_levels: List[FormalityLevel] = Field(default_factory=lambda: list(FormalityLevel))
    
    # File paths
    output_dir: str = Field(default="data/output", description="Output directory for generated data")
    log_dir: str = Field(default="logs", description="Directory for log files")
    cache_dir: str = Field(default="data/cache", description="Directory for caching intermediate results")
    ...
    @classmethod
    def validate_transformation_ratio(cls, v):
        if not 0.0 <= v <= 1.0:
    ...
    def content_matrix_size(self) -> int:
        ...
        return len(self.domains) * len(self.bias_types) * len(self.time_epochs) * len(self.formality_levels)
