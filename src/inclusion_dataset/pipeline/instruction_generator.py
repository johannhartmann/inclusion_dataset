...
Wichtig: Die Instruction muss aus der Textanalyse abgeleitet werden, nicht aus vorgefertigten Templates!"""
        ...
        self.transformation_instruction = (
            "Erstelle eine vollständige inklusive Umformulierung des Textes"
        )
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte für kontextuelle Aufgabenerstellung ohne Templates.",
                    },
            ...
            if parsed_response:
                ...
                return self._generate_fallback_instruction(
                    text, domain, bias_type, task_type
                )

        except Exception as e:
            return self._generate_fallback_instruction(
                text, domain, bias_type, task_type, str(e)
            )
    ...
    def _generate_response(
        self, text: str, instruction: str, task_type: str, analysis: Dict
    ) -> str:
...
Bewerte den Text (1-10) und erkläre strukturiert: Problem → Begründung → Verbesserungsvorschlag"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Du bist ein Experte für inklusive Sprache.",
                    },
            ...
            if task_type == TaskType.TRANSFORMATION.value:
                ...
                return (
                    "Bewertung: 5/10. Verbesserungen in der inklusiven Sprache möglich."
                )

    def _generate_fallback_instruction(
        self,
        text: str,
        domain: str,
        bias_type: str,
