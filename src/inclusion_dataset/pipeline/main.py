...
from distilabel.steps import Step, LoadDataFromDicts, StepInput
...
class InclusionDatasetPipeline:
    ...
    def __init__(self, config: Config):
        ...
        logger.info(f"Initialized InclusionDatasetPipeline with {config.total_samples} target samples")
    ...
    def create_distilabel_pipeline(self, sample_size: int = 10) -> Pipeline:
        ...
        with Pipeline(pipeline_name) as pipeline:
            ...
            # Connect steps
            load_data >> content_step >> instruction_step >> quality_step >> diversity_step >> format_step
    ...
    def _generate_content_matrix_data(self, sample_size: int) -> List[Dict[str, Any]]:
        ...
        selected_combinations = combinations[:sample_size] if len(combinations) >= sample_size else combinations * (sample_size // len(combinations) + 1)
    ...
    def _generate_dry_run_report(self, dataset) -> Dict[str, Any]:
        ...
        if len(dataset) > 0:
            ...
            if "quality_score" in samples[0]:
                ...
                report["quality_metrics"] = {
                    ...
                    "passed_samples": sum(1 for s in scores if s >= self.config.min_quality_score)
                }
            
            # Diversity metrics
            instructions = [s.get("instruction", "") for s in samples if s.get("instruction")]
            if instructions:
                from ..validators.diversity_metrics import DiversityMetrics
                from ..validators.template_detector import TemplateDetector
                ...
                diversity_results = diversity_metrics.calculate_lexical_diversity(instructions)
...
class InstructionGenerationStep(Step):
    ...
    @property
    def inputs(self) -> List[str]:
        return ["generated_text", "content_metadata", "domain", "bias_type", "task_type"]
    ...
    def process(self, *inputs: StepInput) -> Generator[List[Dict[str, Any]], None, None]:
        ...
        for input_batch in inputs:
            ...
            if instructions:
                ...
                diversity_passed = (
                    diversity_results["ttr"] >= 0.4 and 
                    not template_results["templates_detected"]
                )
            else:
    ...
    def process(self, *inputs: StepInput) -> Generator[List[Dict[str, Any]], None, None]:
        ...
        for input_batch in inputs:
            ...
            for input_data in input_batch:
                ...
                logger.info(f"Sample status: quality_passed={quality_passed}, diversity_passed={diversity_passed}")
                
                # For dry run, be less restrictive - accept if quality passed OR if we have valid content
                has_content = (input_data.get("instruction", "") and 
                              input_data.get("input", "") and 
                ...
                if quality_passed or has_content:
                    ...
                    logger.warning(f"Sample rejected: quality_passed={quality_passed}, has_content={has_content}")
