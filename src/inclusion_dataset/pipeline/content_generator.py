...
from ..config.settings import Config, Domain, BiasType, TimeEpoch, FormalityLevel, TaskType
...
Generiere NUR den Text, ohne Kommentare oder Erklärungen."""
    ...
    def generate_content(self, domain: str, bias_type: str, time_epoch: str, 
                        formality: str, task_type: str) -> Dict[str, Any]:
        ...
        prompt_params = {
            ...
            "language_register": domain_config.language_registers.get(formality_enum, "Standard"),
            "scenarios": ", ".join(random.sample(domain_config.common_scenarios, min(3, len(domain_config.common_scenarios)))),
            "time_specific_language": ", ".join(BiasTypeConfig.get_problematic_language(bias_enum, epoch_enum)[:3])
        }
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "Du bist ein Experte für realistische Textgenerierung."},
...
Füge weitere {params['min_length'] - len(text.split())} Wörter hinzu, die zum Kontext ({params['domain']}, {params['time_epoch']}) passen."""
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.config.teacher_model,
                messages=[
                    {"role": "system", "content": "Du erweiterst Texte kontextuell passend."},
            ...
            return text + f" Weitere Details zu diesem Thema im Kontext von {params['domain']} sind besonders relevant."
    
    def _generate_fallback_content(self, params: Dict[str, Any], error: str) -> Dict[str, Any]:
        ...
        base_text = fallback_templates.get(domain_key, "Dies ist ein Beispieltext für die Generierung.")
